<template>
  <div class="KeyIndicators" ref="KeyIndicators">
    <div class="title">重点指标数据</div>
    <Tabs value="name1">
      <TabPane label="血常规" name="name1">
        <Row  :gutter="10">
          <Col span="12" v-for="(it, i) in data.data_1" :key="i"><img :src="'http://118.31.38.185'+it.imageUrl"  width="100%" @click="show(it.imageUrl)"/></Col>
        </Row>
      ...
      <TabPane label="生化指标" name="name2">
        <Row  :gutter="10">
          ...
          <Row  :gutter="10">
            <Col span="12" v-for="(it, i) in data.data_2" :key="i"><img :src="'http://118.31.38.185'+it.imageUrl" width="100%" @click="show(it.imageUrl)"/></Col>
          </Row>
        </Row>
      ...
      <TabPane label="心脏彩超" name="name3">
        <Row  :gutter="10">
          <Row  :gutter="10">
            <Col span="12" v-for="(it, i) in data.data_3" :key="i"><img :src="'http://118.31.38.185'+it.imageUrl" width="100%" @click="show(it.imageUrl)"/></Col>
          </Row>
        </Row>
      ...
      <TabPane label="心肌标志物" name="name4">
        <Row  :gutter="10">
          <Row  :gutter="10">
            <Col span="12" v-for="(it, i) in data.data_4" :key="i"><img :src="'http://118.31.38.185'+it.imageUrl" width="100%" @click="show(it.imageUrl)"/></Col>
          </Row>
        </Row>
      ...
      <TabPane label="大便常规" name="name5">
        <Row  :gutter="10">
          <Row  :gutter="10">
            <Col span="12" v-for="(it, i) in data.data_5" :key="i"><img :src="'http://118.31.38.185'+it.imageUrl" width="100%" @click="show(it.imageUrl)"/></Col>
          </Row>
        </Row>
      ...
      <TabPane label="冠脉造影" name="name6">
        <Row>
          <Row  :gutter="10">
            <Col span="12" v-for="(it, i) in data.data_6" :key="i"><img :src="'http://118.31.38.185'+it.imageUrl" width="100%" @click="show(it.imageUrl)"/></Col>
          </Row>
        </Row>
        <uploadImg :dlength="data.data_6.length" @getImgUrl="imgUpSuccess" :type="6" :num="5"/>
      </TabPane>
      <TabPane label="表格" name="name7"><Datatable/></TabPane>
    </Tabs>
...
<script>
  ...
  import Datatable from './Datatable.vue';
    ...
    export default {
      name: "key-indicators",
      components: { Datatable, uploadImg },
      data() {
        return {
          ADIMG: '',
          showF: false,
