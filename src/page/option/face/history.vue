    ...
    <div class="list">
      <div class="item" v-for="(it, i) in data">
        <Row>
          ...
          <Col span="20">
            <div class="date"><span class="c9">预约时间：</span>{{it.visiteStartDate}}</div>
            <div class="date"><span class="c9">面诊时间：</span>{{it.faceTime}}</div>
  ...
  export default {
    ...
    methods: {
      getList() {
        this.$ajax({
          method: 'get',
          url:findTeamCustomerHealServiceNum()+'?healthServiceId=9&teamId=' + this.teamId,
          dataType: 'JSON',
