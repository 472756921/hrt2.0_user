<template>
  <div class="history_content" ref="history_content">
    ...
    <div class="list">
      <div class="item" v-for="(it, i) in data"  @click="continue_q(it)">
        <Row>
          <Col span="4">
            <Avatar size="large" style="background: #43a3f0">{{tName}}</Avatar>
          </Col>
          <Col span="20">
            <div class="date"><span class="c9">提问时间：</span>{{it.visiteStartDate}}</div>
            <div class="teamName"><span class="c9">医生团队：</span>{{it.teamName}}</div>
  ...
  export default {
    ...
    methods: {
      getList() {
        this.$ajax({
          method: 'get',
          url:findTeamCustomerHealServiceNum()+'?healthServiceId=7&teamId=' + this.teamId,
          dataType: 'JSON',
      ...
      continue_q(it) {
        if(it.status == 1){
          this.$Message.warning('问答已关闭，不能再次进入');
          return false;
        }
        this.$router.push({name: 'q_conversation', params: {id:it.id}});
      },
    },
  }
</script>
